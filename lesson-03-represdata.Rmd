...
<a title="Hamid Hajihusseini, CC BY 3.0 &lt;https://creativecommons.org/licenses/by/3.0&gt;, via Wikimedia Commons" href="https://commons.wikimedia.org/wiki/File:Eurasian_Cranes_migrating_to_Meyghan_Salt_Lake.jpg"><img width="512" alt="Eurasian Cranes migrating to Meyghan Salt Lake" src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/0b/Eurasian_Cranes_migrating_to_Meyghan_Salt_Lake.jpg/512px-Eurasian_Cranes_migrating_to_Meyghan_Salt_Lake.jpg"></a>
...
To build the framework, let's look at the core 7 Guerrilla Analytics Principles:
 ...
 >3. Automate (_everything - my addition_) with program code
...
This principle is simple: storage costs are low these days so there is no need to spend a lot of time on administrating files. 
 ...
 <!-- When installing open source software always check the checksums of the downloaded file. More on this topic in the next paragraph -->
...
So when managing files in projects adhere to the following guidelines
 ...
 1. **Create a separate folder for each analytics project** . Keep the unit of a project small. So a larger research project can usually be subdivided into smaller projects. Create an analysis project for each sub-project. Don't try to fit it all together.
 1. **Do not deeply nest folders** (max 2-3 levels, so no *project/documentation/applications/metc/first_communication/input/stuff/morestuff/mail.txt*)
 ...
 1. **Store each dataset in its own sub-folder.** I usually work like the example below. Each dataset I receive goes in a numbered folder in the `data-raw` folder. If I recieve an updated file, I add the old file to a new folder called `v01` inside the orginal `Data..` folder. The updated file replaces the `v01` file. If then you recieve yet another update, you can repeat this trick with a `v02` folder inside the `D..` folder. In this way, the file directly in the `D..` folder is always the latest updated version. The README.txt file containing data about the data (see \@ref(metadata1)) lives also in the root of the `Data..` folder.
 ...
 1. **Do not manually edit data source files.** in fact **Never, ever change a value inside a file**. You can (_almost_) always solve this using code. 
 1. **In code, use relative paths** (here::here etc)
 1. **Use one R project per project** (in RStudio -> RStudio Project) Don't work outside of projects, and don't reuse projects for other stuff.
...
Take heed of these pointers:
 ...
 >- Store (R) functions in R in a "./R" folder 
  <!--and document them with [Roxygen2](https://cran.r-project.org/web/packages/roxygen2/vignettes/roxygen2.html) -->

<!-- XXX Roxygen2 heel graag, maar dan moeten we dat uitleggen in de cursus. Voor nu doorgeschoven naar volgend jaar. -->
...
```{r,  dpi=100, echo=FALSE, fig.cap="A short example of a README.txt file"}
knitr::include_graphics(
  here::here(
    "images",
    "readme_updated.png"))
...
(Actually, we will ask R to generate some data that looks like it was imported from excel instead, because we [didn't feel like](https://im.indiatimes.in/photogallery/2015/Mar/1328016_1437683070.jpg?w=1200&h=900&cc=1) copy-pasting 4x100 numbers to make an excelfile...)
...
<!-- There are two solutions that could be provided here. One is more laborious to achieve manually (solution 2) than the other (solution 1). If there is no expertise for scripting reformatting and cleaning up data is available in the research group, I recommend solution 2. -->

<!-- ### Solution 1 (requires programming skills) -->


<!-- Add meta data and variable information to a separate table. The strategy is to keep all experimentally relevant data that is related to the experimental design in one sheet and all additional data (metadata or also called 'data about the data').  -->
...
<!-- Make `/doc` and `/data` and `data_raw` folders now for all (at least) 3 of your projects this semester. Add a `data_log.xlsx` for the projecticum folder. -->
...
```{r flipperplot, message=FALSE, error=FALSE, fig.cap="Example of inconsistent data entry"}
library(ggplot2)
...
```{r, echo=TRUE, fig.cap="Graph displaying the flipper length per factor level of Sex"}
data_penguins %>%
...
Or check out a ggplot method [here.](https://towardsdatascience.com/missing-value-visualization-with-tidyverse-in-r-a9b0fefd2246)
...
### Tidy template
A tidy data template you may want to use is available [here](https://github.com/DataScienceILC/tlsc-dsfb26v-20_workflows/raw/main/data/CE.LIQ.FLOW.062_Tidydata.xlsx)
...
(A) 
 1. Look at your RStudio project that you created for the DAUR-II assignments
 1. download your folders (not the HUGE datafiles and kraken-database!) and rearrange your project according the Guerilla principles explained above
 1. for the data... folders: provide only a readme.txt with alle the info you would normally put there + where the data is stored
 1. Provide study documentation and meta data. 
